...
void applyConfigData(void) {
	...
	gDipSwitch0  = cfg.dipSwitchDD0;
	gDipSwitch1  = cfg.dipSwitchDD1;
	gDipSwitch2  = cfg.dipSwitchDD2;
	gDipSwitch3  = cfg.dipSwitchDD3;
}
...
void updateConfigData(void) {
	...
	cfg.dipSwitchDD0 = gDipSwitch0;
	cfg.dipSwitchDD1 = gDipSwitch1;
	cfg.dipSwitchDD2 = gDipSwitch2;
	cfg.dipSwitchDD3 = gDipSwitch3;
}
...
void initSettings() {
	...
	cfg.dipSwitchDD1 = 0x85;		// Lives, cabinet & demo sound.
...
bool loadRoms(int gameNr, bool doLoad) {
	return loadACRoms(ROM_Space, ddribbleGames, gameNr, ARRSIZE(ddribbleGames), doLoad);
}
...
bool loadGame(int gameNr) {
	...
	selectedGame = gameNr;
	strlcpy(currentFilename, ddribbleGames[selectedGame].gameName, sizeof(currentFilename));
	setEmuSpeed(0);
	loadCart(gameNr,0);
